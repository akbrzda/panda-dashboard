const { validatePeriodParams, validatePositiveInteger, validateEnum, validateGeoJsonFeatureCollection } = require("./requestValidation");
...
function validateMenuAbcParams(payload = {}) {
  ...
  return {
    ...
    normalized: {
      ...base.normalized,
      abcGroup: String(payload.abcGroup || "all")
        .trim()
        .toLowerCase(),
...
function validateProductionForecastParams(payload = {}) {
  ...
  if (!analysisWindowValidation.isValid) return analysisWindowValidation;
