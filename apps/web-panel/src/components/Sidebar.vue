<template>
  ...
  >
    ...
    <nav class="flex-1 overflow-y-auto p-2">
      <section v-for="section in menuSections" :key="section.title" class="mb-4 space-y-1">
        <p v-if="sidebarStore.open" class="px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">
          {{ section.title }}
        </p>
  ...
  <transition name="fade">
    <div v-if="sidebarStore.openMobile" class="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm md:hidden" @click="sidebarStore.closeMobile" />
  </transition>
  ...
  >
    <div class="flex h-12 items-center justify-between px-1">
      ...
      <Button type="button" variant="ghost" size="icon" class="h-8 w-8 text-muted-foreground" @click="sidebarStore.closeMobile">
        <X class="h-4 w-4" />
      </Button>
    </div>
